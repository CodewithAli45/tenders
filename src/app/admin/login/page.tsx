      ...
      <section className="w-full max-w-md overflow-hidden rounded-3xl border border-white/10 bg-slate-900 shadow-2xl shadow-black/40">
        ...
        <form onSubmit={connect} className="space-y-5 p-8">
          ...
          <div className="relative">
            ...
            <input id="admin-password" value={password} onChange={(event) => setPassword(event.target.value)} type="password" autoComplete={setupRequired ? "new-password" : "current-password"} minLength={setupRequired ? 8 : undefined} required autoFocus placeholder={setupRequired ? "At least 8 characters" : "Enter admin password"} className="h-12 w-full rounded-xl border border-slate-700 bg-slate-800 pl-11 pr-4 text-sm outline-none transition focus:border-primary focus:ring-4 focus:ring-primary/20" />
          </div>
          {setupRequired && <>
            ...
            <div className="relative">
              ...
              <input id="confirm-password" value={confirmation} onChange={(event) => setConfirmation(event.target.value)} type="password" autoComplete="new-password" minLength={8} required placeholder="Re-enter password" className="h-12 w-full rounded-xl border border-slate-700 bg-slate-800 pl-11 pr-4 text-sm outline-none transition focus:border-primary focus:ring-4 focus:ring-primary/20" />
            </div>
          </>}
          {error && <p role="alert" className="rounded-lg bg-danger/10 px-3 py-2 text-sm text-danger">{error}</p>}
          <button disabled={isSubmitting || setupRequired === null} className="flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-primary text-sm font-bold text-white transition hover:bg-[#9f5829] disabled:cursor-not-allowed disabled:opacity-70">
