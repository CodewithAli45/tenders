...
export function AdminDashboard({ onClose, isModal = false }: AdminDashboardProps) {
  ...
  function closeEditor() { setMode("list"); setViewingTender(null); }

  return <div className={`flex h-full w-full overflow-hidden bg-background text-foreground ${isModal ? "rounded-2xl" : ""}`}>
    <aside className={`flex w-64 shrink-0 flex-col border-r border-border bg-card ${mode !== "list" || viewingTender ? "hidden lg:flex" : ""}`}>
      <div className="flex items-center gap-3 border-b border-border px-5 py-6"><div className="grid h-9 w-9 place-items-center rounded-xl bg-primary text-white"><LayoutDashboard className="h-5 w-5" /></div><div><p className="font-bold">Admin panel</p><p className="text-xs text-muted-foreground">Tender records</p></div></div>
      ...
      <AnimatePresence mode="wait">
        ...
        <motion.div key="list" className="flex h-full flex-col" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
          ...
          <section className="flex-1 overflow-auto p-5 md:p-8"><div className="overflow-hidden rounded-2xl border border-border bg-card">
            ...
            {section === "organizations" && <OrganizationTable organizations={organizations} loading={loading} />}
