...
export function NewOrganizationForm({ onClose, onSuccess, organization }: { onClose: () => void; onSuccess: (organization: unknown) => void; organization?: Organization }) {
  const isEditing = !!organization;
  const [data, setData] = useState({ name: organization?.name || "", details: organization?.details || "", contactPerson: organization?.contactPerson || "", email: organization?.email || "", phone: organization?.phone || "", address: organization?.address || "" });
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      if (isEditing) {
        const response = await fetch("/api/organizations", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ _id: organization._id, ...data }) });
  ...
  return <div className="h-full overflow-y-auto bg-background p-6 md:p-8">
    <div className="mx-auto max-w-3xl">
      <div className="mb-8 flex items-center justify-between border-b border-border pb-5">
        <div className="flex items-center gap-3"><div className="grid h-10 w-10 place-items-center rounded-xl bg-primary/10 text-primary"><Building2 className="h-5 w-5" /></div><div><h2 className="text-xl font-bold">{isEditing ? "Edit organization" : "Add organization"}</h2><p className="text-sm text-muted-foreground">{isEditing ? "Update the organization details below." : "Only the name is required. You can add details later."}</p></div></div>
      ...
      <form onSubmit={submit} className="space-y-5">
        ...
        <div><label className="mb-2 block text-sm font-medium">Organization details</label><textarea value={data.details} onChange={(e) => setData({ ...data, details: e.target.value })} className="min-h-28 w-full rounded-xl border border-border bg-background p-3 text-sm outline-none focus:border-primary focus:ring-4 focus:ring-primary/10" placeholder="Describe the department, work scope, or relevant details (optional)." /></div>
